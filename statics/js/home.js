...
var MAC={
    ...
    'Verify': {
        ...
        'Click': function(){//点击刷新
            $('body').on('click', 'img.mac_verify_img', function(){
                $(this).attr('src', maccms.path +'/verify/index.html?');
            });
        },
        'Refresh':function(){
            $('.mac_verify_img').attr('src', maccms.path +'/verify/index.html?');
        },
        'Show':function(){
    ...
    'Hits': {
        'Init':function() {
            ...
            $.ajax({
                type: 'get',
                url: maccms.path + '/ajax/hits?mid='+$that.attr("data-mid")+'&id='+$that.attr("data-id")+'&type=update',
                timeout: 5000,
                dataType: 'json',
                error: function () {
    ...
    'Score': {
        'Init':function(){
            ...
            $.ajax({
                type: 'post',
                url: maccms.path+'/ajax/score?mid='+ $('.mac_score').attr('data-mid') +'&id=' +$('.mac_score').attr('data-id'),
                timeout: 5000,
                error: function(){
        ...
        'Submit':function(){
            ...
            $.ajax({
                type: 'post',
                url: maccms.path+'/ajax/score?mid='+$('.mac_score').attr('data-mid')+'&id='+$('.mac_score').attr('data-id') + '&score='+ $s,
                timeout: 5000,
                error: function(){
        ...
        'Init':function(){
            ...
            $('.mac_star').raty({
                ...
                click: function(score, evt) {
                    $.ajax({
                        type: 'get',
                        url: maccms.path+'/ajax/score?mid='+$('.mac_star').attr('data-mid')+'&id='+$('.mac_star').attr('data-id')+'&score='+(score*2),
                        timeout: 5000,
                        dataType:'json',
                        error: function(){
        ...
        'Init':function(){
            $('body').on('click', '.digg_link', function(e){
                ...
                if($that.attr("data-id")){
                    $.ajax({
                        url: maccms.path + '/ajax/digg.html?mid='+$that.attr("data-mid")+'&id='+$that.attr("data-id")+'&type='+$that.attr("data-type"),
                        cache: false,
                        dataType: 'json',
                        success: function($r){
    ...
    'Gbook':{
        ...
        'Show':function($page){
            $.ajax({
                type: 'post',
                url: maccms.path+'/gbook/index?page='+$page,
                timeout: 3000,
                error: function(){
        ...
        'Submit':function(){
            ...
            $.ajax({
                type: 'post',
                url: maccms.path + '/gbook/saveData',
        ...
        'Report':function(name){
            MAC.Pop.Show('#modal-seport',maccms.path+'/gbook/report?name=' + encodeURIComponent(name),function($r){
                $('body').on('click', '#gbook_submit', function(e){
	            	MAC.Gbook.Submit();
		        });
            });
    ...
    'Ulog':{
        ...
        'Get':function(){
            $.ajax({
                url: maccms.path+'/user/ajax_ulog/?ac=list',
                type:'GET',
                cache: false,
                dataType: 'json',
                success: function($r){
    ...
    'User':{
        ...
        'Login':function(){
            ...
            MAC.Pop.Show('#modal-login',maccms.path+'/user/'+ac,function($r){
                $('body').on('click', '#login_form_submit', function(e){
                    $.ajax({
                        type: 'POST',
                        url: maccms.path + '/user/login',
        ...
        'BuyPopedom':function(o){
            ...
            if($that.attr("data-id")){
                if (confirm('您确认购买此条数据播放权限吗？')) {
                    $.ajax({
                        url: maccms.path + '/user/ajax_buy_popedom.html?id=' + $that.attr("data-id") + '&sid=' + $that.attr("data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr("data-type"),
                        cache: false,
                        dataType: 'json',
                        success: function ($r) {
                            $that.addClass('disabled');
    ...
    'Comment':{
        ...
        'Init':function(){
            ...
            $('body').on('click', '.comment_report', function(e){
                ...
                if($(this).attr("data-id")){
                    $.ajax({
                        url: maccms.path + '/comment/report.html?id='+$that.attr("data-id"),
                        cache: false,
                        dataType: 'json',
                        success: function($r){
                            $that.addClass('disabled');
        ...
        'Show':function($page){
            if($(".mac_comment").length>0){
                $.ajax({
                    type: 'get',
                    url: maccms.path + '/comment/ajax.html?rid='+$('.mac_comment').attr('data-id')+'&mid='+ $('.mac_comment').attr('data-mid') +'&page='+$page,
                    timeout: 5000,
                    error: function(){
        ...
        'Submit':function($o){
            ...
            $.ajax({
                type: 'post',
                url: maccms.path + '/comment/saveData',
